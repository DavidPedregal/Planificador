...
describe('calendarRepository', () => {
    describe('createCalendar', () => {
        it('should create a calendar with all fields', async () => {
            ...
            expect(calendar.userId).toBe(mockCalendar.userId);
    ...
    describe('findCalendarForUser', () => {
        it('should find a calendar by userId and calendarId', async () => {
            ...
            const found = await CalendarRepo.findCalendarForUser(mockCalendar.userId, created._id.toString());
        ...
        it('should return null if calendar belongs to another user', async () => {
            const created = await CalendarRepo.createCalendar(mockCalendar);
            const found = await CalendarRepo.findCalendarForUser('otherUser', created._id.toString());
        ...
        it('should throw RepositoryError if calendarId format is invalid', async () => {
            await expect(
                CalendarRepo.findCalendarForUser(mockCalendar.userId, 'invalid_id')
    ...
    describe('findCustomCalendarsForUser', () => {
        it('should return only non-system calendars for the user', async () => {
            ...
            const calendars = await CalendarRepo.findCustomCalendarsForUser(mockCalendar.userId);
        ...
        it('should not return calendars from other users', async () => {
            await CalendarRepo.createCalendar({ ...mockCalendar, userId: 'otherUser' });
            const calendars = await CalendarRepo.findCustomCalendarsForUser(mockCalendar.userId);
            expect(calendars).toHaveLength(0);
        });
    });
    ...
    describe('findSystemCalendarsForUser', () => {
        it('should return only system calendars for the user', async () => {
            ...
            const calendars = await CalendarRepo.findSystemCalendarsForUser(mockCalendar.userId);
    ...
    describe('deleteCalendar', () => {
        it('should delete an existing calendar', async () => {
            const created = await CalendarRepo.createCalendar(mockCalendar);
            await CalendarRepo.deleteCalendar(mockCalendar.userId, created._id.toString());
        ...
        it('should not delete a calendar belonging to another user', async () => {
            const created = await CalendarRepo.createCalendar(mockCalendar);
            await CalendarRepo.deleteCalendar('otherUser', created._id.toString());
        ...
        it('should throw RepositoryError if calendarId format is invalid', async () => {
            await expect(
                CalendarRepo.deleteCalendar(mockCalendar.userId, 'invalid_id')
    ...
    describe('updateCalendar', () => {
        ...
        it('should return null if calendar does not exist', async () => {
            ...
            const updated = await CalendarRepo.updateCalendar(mockCalendar.userId, fakeId, { name: 'X' });
            expect(updated).toBeNull();
        });

        it('should throw RepositoryError if calendarId format is invalid', async () => {
            await expect(
                CalendarRepo.updateCalendar(mockCalendar.userId, 'invalid_id', { name: 'X' })
